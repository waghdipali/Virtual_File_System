...
#define READ 1
#define WRITE 2      // Geive permission as 3 For Both Read & Write
...
typedef struct inode      // 86 bytes allocated for this block
{
    ...
    int permission;  // 1  23      read write permission
...
void man(char *name)
{
    ...
    if(strcmp(name, "create") == 0)
    {
        ...
        printf("Usage : create File_name mode\n");
    }
    ...
    {
        printf("Description : used to write into regular\n");
...
void CreateDILB()
{
    ...
    while(i <= MAXINODE)
    {
        newn = (PINODE)malloc(sizeof(INODE));   //86 Bytes Of Memory
...
void InitialiseSuperBlock()
{
    ...
    while(i < MAXINODE)
    {
        UFDTArr[i].ptrfiletable = NULL;      // this loop is used to set all the pointer at null
        i++;
    }
...
int fstat_file(int fd)
{
    ...
    printf("\n----------Statistical Inoformation about file-----------\n");
    printf("File name : %s\n",temp->FileName);
    ...
    while(1)
    {
        ...
        else if(count == 3)
        {
            if(strcmp(command[0],"create") == 0)
            {
                ...
                if(ret >= 0)
                {
                    printf("File is successfully created eith file descriptor : %d\n",ret);
                }
                if(ret == -1)
                {
